...
import ac_pysmac.remote_smac
from .utils.multiprocessing_wrapper import MyPool
from ac_pysmac.utils.java_helper import check_java_version, smac_classpath


class SMAC_optimizer(object):
    """
    The main class of ac_pysmac instanciated by the user.
    ...
    def __init__(self, t_limit_total_s=None, mem_limit_smac_mb=None, working_directory=None, persistent_files=False,
                 debug=False):
        """
        ...
        :param debug: set this to true for debug information (ac_pysmac and SMAC itself) logged to standard-out.
        :type debug: bool
        """
    ...
    def minimize(self, func, max_evaluations, parameter_dict,
        ...
        :param mem_limit_function_mb: sets the memory limit for your function (value in MB). ``None`` means no restriction. Be aware that this limit is enforced for each SMAC run separately. So if you have 2 parallel runs, ac_pysmac could use twice that value (and twice the value of mem_limit_smac_mb) in total. Note that due to the creation of the subprocess, the amount of memory available to your function is less than the value specified here. This option exists mainly to prevent a memory usage of 100% which will at least slow the system down.
        ...
        pcs_string, parser_dict = ac_pysmac.remote_smac.process_parameter_definitions(parameter_dict)
        ...
        if (train_instance_features is not None):
            # print(self.smac_options)
            # exit()
            with open(self.smac_options['feature_file'], 'w') as fh:
                # write a header
                tmp = ['instance_name'] + list(
                    map(lambda i: 'feature{}'.format(i), range(len(train_instance_features[0]))))
                fh.write(",".join(tmp));
                fh.write("\n");
        ...
        scenario_options = {'algo', 'algo-exec', 'algoExec',
                            ...
                            'intra-obj', 'intra-instance-obj', 'overall-obj', 'intraInstanceObj', 'overallObj',
                            'overall_obj', 'intra_instance_obj',
                            'algo-cutoff-time', 'target-run-cputime-limit', 'target_run_cputime_limit', 'cutoff-time',
                            ...
                            'instances', 'instance-file', 'instance-dir', 'instanceFile', 'i', 'instance_file',
                            'instance_seed_file',
                            'test-instances', 'test-instance-file', 'test-instance-dir', 'testInstanceFile',
                            'test_instance_file', 'test_instance_seed_file',
        ...
        assert all(map(os.path.exists, [additional_options_fn, scenario_fn, self.smac_options['pcs-file'],
                                        self.smac_options[
                                            'instances']])), "Something went wrong creating files for SMAC! Try to specify a \'working_directory\' and set \'persistent_files=True\'."

        # create a pool of workers and make'em work
        pool = MyPool(num_procs)
        argument_lists = [
            [scenario_fn, additional_options_fn, s, func, parser_dict, self.__mem_limit_smac_mb, smac_classpath(),
             num_train_instances, mem_limit_function_mb, t_limit_function_s, self.smac_options['algo-deterministic'],
             java_executable, timeout_quality] for s in seed]

        pool.map(ac_pysmac.remote_smac.remote_smac_function, argument_lists)
