...
import time
import ac_pysmac.utils.utils_processes_api as utils_processes_api
...
def process_single_parameter_definition(name, specification):
    ...
    if specification[0] in {'real', 'integer'}:
        ...
        if specification[0] == 'integer':
            if (type(specification[1][0]) != int) or (type(specification[1][1]) != int) or (
                        type(specification[2]) != int):
                raise ValueError(
                    "Bounds and default value of integer parameter {} have to be integer types!".format(name))
...
def remote_smac_function(only_arg):
    ...
    rather part of the internals of ac_pysmac. Due to the limitations of the
    ...
    try:
        ...
        while True:
            ...
            wrapped_function = utils_processes_api.enforce_limits(
            ...
            if res is not None:
                try:
                    logger.debug('iteration %i:function value %s, computed in %s seconds' % (
                        num_iterations, str(res), str(res['runtime'])))
                except (TypeError, AttributeError, KeyError, IndexError):
                    logger.debug(
                        'iteration %i:function value %s, computed in %s seconds' % (num_iterations, str(res), cpu_time))
                except:
                    raise
            else:
