...
def read_sate_run_folder(directory, rar_fn="runs_and_results-it*.csv", inst_fn="instances.txt",
                         feat_fn="instance-features.txt", ps_fn="paramstrings-it*.txt"):
    ...
    :type inst_fn: str
    :param feat_fn: name of the instance feature file. If this file is not found, ac_pysmac assumes no instance features.
    :type feat_fn: str
...
def state_merge(state_run_directory_list, destination,
    ...
    for directory in state_run_directory_list:
        ...
        for i in range(len(inst_names)):
            ...
            if not inst_names[i][0] in instances:
                ...
                instances[inst_names[i][0]]['features'] = inst_feats[
                    inst_names[i][0]] if inst_feats is not None else None
                instances[inst_names[i][0]]['additional info'] = ' '.join(inst_names[i][1:]) if len(
                    inst_names[i]) > 1 else None
                i_insts += 1
            else:
                if (inst_feats is None):
                    if not (instances[inst_names[i][0]]['features'] is None):
                        raise ValueError(
                            "The data contains the same instance name ({}) twice, but once with and without features!".format(
                                inst_names[i]))
                elif not numpy.all(instances[inst_names[i][0]]['features'] == inst_feats[inst_names[i][0]]):
                    raise ValueError(
                        "The data contains the same instance name ({}) twice, but with different features!".format(
                            inst_names[i]))
                pass
    ...
    if header_feats is not None:
        with open(os.path.join(destination, 'instance-features.txt'), 'w') as fh:
            ...
            sorted_features = [
                (instances[inst]['index'], inst + ',' + ",".join(list(map(str, instances[inst]['features'])))) for inst
                in instances]
